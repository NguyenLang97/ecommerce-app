...
function DiscountList() {
    ...
    return (
        <div className={`Discount-List box-sha-home d-flex flex-direction-column bg-${indexHeader}`}>
            {/* menu header */}
                ...
                {list.map((item, index) => {
                    ...
                    return (
                        <div
                            ...
                            className={index !== indexHeader ? className : className + ' active'}
                        >
                            <h2>{item.title}</h2>
            ...
            <Row className="flex-grow-1">
                {/* countdown */}
                <Col span={24} md={6} className="d-flex flex-direction-column justify-content-center countdown">
                    <Countdown timeTillDate={list[indexHeader].deadline} timeFormat="MM DD YYYY, h:mm a" />
                </Col>
                {/* product list */}
                <Col span={24} md={18}>
                    <RelatedProduct type={list[indexHeader].type} span={{ span: 24, xs: 24, sm: 12, md: 12, lg: 6, xl: 6, xxl: 6 }} />
                </Col>
            </Row>
        </div>
    )
